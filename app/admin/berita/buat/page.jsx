...
import { createBerita, getKategori, uploadImage } from '@/lib/api';
    ...
    useEffect(() => {
        const loadCategories = async () => {
            try {
                ...
                const cats = res.data || [];
                setKategoris(cats);
                // Tidak ada default selection untuk multi-kategori
            } catch (err) {
                console.error(err);
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const data = {
                ...
                category: selectedCategories[0], // Backward compat: category pertama jadi primary
            };
            const res = await createBerita(token, data);
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 md:mb-8 border-b border-[#2a2a3a] pb-4 gap-3">
                <div>
                    <h1 className="text-xl md:text-2xl font-extrabold text-white flex items-center gap-3">
                        <span className="text-[#e63946]">✍️</span> Tulis Berita Baru
                    </h1>
                    <p className="text-xs md:text-sm text-[#8888aa] mt-1">Buat artikel berita baru menggunakan editor teks kaya.</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-5 md:gap-8">
                    ...
                    <div className="space-y-6">
                        <div className="bg-[#16161f] border border-[#2a2a3a] rounded-xl p-5 space-y-5">
                                        ...
                                        {kategoris.map((k) => (
                                            <label key={k._id} className={`flex items-center gap-2.5 px-3 py-2 rounded-lg cursor-pointer border transition-all ${
                                                ...
                                                <input
                                                    ...
                                                    onChange={() => toggleCategory(k.name)}
                            ...
                            >
                                {loading ? 'Menyimpan Berita...' : 'Simpan Berita'}
                            </button>
                        </div>
                    </div>
                </form>
