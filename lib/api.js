...
export async function getBeritaBySlug(slug) {
    const res = await fetch(getFetchUrl(`/post-berita/${slug}`), { cache: 'no-store' });
...
// ======== ADMIN BERITA ========
export async function getBeritaAdmin(token, { page = 1, limit = 10 } = {}) {
    const res = await fetch(getFetchUrl(`/post-berita?page=${page}&limit=${limit}`), {
